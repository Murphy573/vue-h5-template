<template>
  <div ref="richtextRef" class="rich-text">
    <!-- 京东富文本 -->
    <div
      v-if="isJd"
      v-html="richtext"
    ...
    <!-- 默认我们的富文本 -->
    <div v-else v-html="richtext" class="default-content"></div>
  </div>
</template>
  ...
  methods: {
    startRender() {
      setTimeout(() => {
        ...
        if (
          _jdWapper &&
          _jdWapper.offsetHeight &&
          this.jdOffsetHeight >= _jdWapper.offsetHeight
        ) {
          _rich.style.height =
            _jdWapper.offsetHeight * this.cmpt_jdScaleFactor + 'px';
        } else {
          this.jdOffsetHeight = _jdWapper.offsetHeight;
          this.startRender();
        }
